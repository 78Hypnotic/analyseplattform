      ...
      <main className="mx-auto w-full max-w-6xl px-5 py-10 pb-24">
        ...
        {home.kind === "admin" ? (
          home.selectedLibrary ? (
            ...
            <EmptyCommunity text="Noch keine Community vorhanden." />
          )
        ) : null}
      </main>
    </>
...
function ThreadList({ threads }: { threads: CommunityThreadSummary[] }) {
  return (
    <section className="mt-8 grid gap-4">
      {threads.map((thread) => (
        <Link key={thread.id} href={`/community/${thread.id}`} className="surface p-5 transition hover:border-[var(--accent)]">
