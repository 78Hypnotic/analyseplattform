...
export default async function CommunitySlugPage({
  ...
  if (isUuid(slug)) {
    const thread = await getCommunityThread(slug);
    if (thread) redirect(`/community/${thread.librarySlug}/threads/${thread.id}`);
  }

  const home = await getCommunityBySlug(slug);
      ...
      <main className="mx-auto w-full max-w-6xl px-5 py-10 pb-24">
        ...
        {home.kind === "community" ? (
          <CommunitySurface
            ...
            communitySlug={home.library.slug}
    ...
    <>
      <header className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
        <div>
          ...
          <h1 className="display-serif mt-3 text-5xl text-[var(--foreground)] sm:text-6xl">Community</h1>
      ...
      {threads.length === 0 ? <EmptyCommunity text="Noch keine Themen in dieser Community." /> : <ThreadList threads={threads} communitySlug={communitySlug} />}
    </>
  );
}

function ThreadList({ threads, communitySlug }: { threads: CommunityThreadSummary[]; communitySlug: string }) {
  return (
    <section className="mt-8 grid gap-4">
      {threads.map((thread) => (
        <Link key={thread.id} href={`/community/${communitySlug}/threads/${thread.id}`} className="surface p-5 transition hover:border-[var(--accent)]">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p className="mt-3 line-clamp-2 max-w-3xl text-sm leading-6 text-[var(--muted)]">{thread.content}</p>
            </div>
