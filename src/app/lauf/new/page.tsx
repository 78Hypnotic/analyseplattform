...
import type { InitialRunInput } from "./running-flow";
...
export default async function NewRunAnalysisPage({
  searchParams,
}: {
  ...
  const context = await getInitialRunContext(athleteId, analysisId);
 ...
 */
async function getInitialRunContext(
  athleteId?: string,
  analysisId?: string,
): Promise<{
  ...
  if (analysisId) {
    const editable = await getEditableAnalysis<RunInput>(analysisId, "run");
...
function toInitialInput(profile: ProfileData): InitialRunInput {
  return {
