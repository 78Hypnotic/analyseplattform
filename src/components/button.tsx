...
function buttonClassName(variant: ButtonProps["variant"]) {
  ...
  if (variant === "primary") {
    return cn(base, "bg-[var(--accent)] text-black hover:bg-[#7ff0e3]");
  }
  if (variant === "ghost") {
    return cn(base, "text-[var(--muted)] hover:text-white");
  }
  return cn(base, "border border-[var(--line)] bg-[var(--panel-2)] text-white hover:border-[var(--accent)]");
}
