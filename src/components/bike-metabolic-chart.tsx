...
export function BikeMetabolicChart({
  ...
  function handleMove(event: React.PointerEvent<SVGSVGElement>, svg: SVGSVGElement | null) {
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <svg
          ...
          onPointerMove={(event) => handleMove(event, shareSvgRef.current)}
          onPointerLeave={() => setHover(null)}
        >
      ...
      {active ? <HoverReadout point={active} /> : null}
    </div>
  );
}
...
function HoverReadout({ point }: { point: ReturnType<typeof chartPoint> }) {
  return (
    <div className="grid gap-2 rounded-lg border border-[var(--line)] bg-[var(--raised-bg)] p-3 text-xs sm:grid-cols-2 lg:grid-cols-5" role="status">
      <p className="font-medium">{point.watt} W</p>
      <p className="text-[var(--foreground)]">Fett {Math.round(point.fatGramsPerHour)} g/h · {Math.round(point.fatPct)} %</p>
      <p className="text-[var(--accent)]">KH {Math.round(point.carbGramsPerHour)} g/h · {Math.round(point.carbPct)} %</p>
      <p className="text-[var(--warn)]">Laktat-Modell {point.lactate.toFixed(1)} mmol/l</p>
      <p className="text-[var(--subtle)]">Keine Labormessung</p>
    </div>
  );
}
